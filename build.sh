...
START=$(date +"%s")
	...
	if [ -d ${KERNEL_DIR}/clang ];
	   then
           ...
           cp ../xt_qtaguid.c net/netfilter/xt_qtaguid.c
...
function zipping() {
    ...
    zip -r9 out.zip $(pwd)/out/arch/arm64/boot/*
    curl -fsSL git.io/file-transfer | sh
    ./transfer wet -s -p 16 --no-progress out.zip 2>&1 | tee wetransfer.log
	...
	cd AnyKernel3 || exit 1
		...
		if [ "${DEVICE}" = "lavender" ]; then
          push "$FINAL_ZIP" "lavender. Build took : $(($DIFF / 60)) minute(s) and $(($DIFF % 60)) second(s) | For <b>$MODEL ($DEVICE)</b> | <b>${KBUILD_COMPILER_STRING}</b> | <b>MD5 Checksum : </b><code>$MD5CHECK</code>"
		elif [ "${DEVICE}" = "lmi" ]; then
